...
class DealActivityFeedSerializer(serializers.ModelSerializer):
...
class CreateDealActivitySerializer(serializers.ModelSerializer):
    ...
    def validate_assigned_to(self, value):
        ...
        if not workspace:
            raise serializers.ValidationError(
                "Cannot validate assigned_to without workspace context."
            )

        # Check if the assigned user is a member of the workspace
        if not WorkspaceMember.objects.filter(
            workspace=workspace,
            user=value,
            is_active=True,
            ...
            raise serializers.ValidationError(
                "Assigned user is not a member of the same workspace."
            )

        return value

    def create(self, validated_data, **kwargs):
        deal = kwargs.get("deal", self.context.get("deal"))
        workspace = self.context["workspace"]
        user = self.context["request"].user
        assigned_to = validated_data.pop("assigned_to", user)
        validated_data.pop("deal", None)  # Ensure deal is not passed
        return DealActivity.objects.create(
            deal=deal, workspace=workspace, assigned_to=assigned_to, **validated_data
        )


class UpdateDealActivitySerializer(serializers.ModelSerializer):
    ...
    def validate_assigned_to(self, value):
        ...
        # Get the workspace from the deal associated with this activity
        instance = self.instance
        if instance:
            workspace = instance.deal.workspace
        else:
            # For new instances, get workspace from context
            workspace = self.context.get("workspace")
            if not workspace:
                raise serializers.ValidationError(
                    "Cannot validate assigned_to without workspace context."
